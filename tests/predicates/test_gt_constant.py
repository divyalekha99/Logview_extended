...
from log_view.predicate import Query, GreaterThanConstant
...
def run_evaluation(log, variable_name, variable_values, len_result_set, len_complement_result_set):
    ...
    filter = Query('query', GreaterThanConstant(variable_name, variable_values))
...
def test_eq_predicate():
    ...
    run_evaluation(initial_source_log, 'Var1', 2, 2, 5)
    check_as_string('Var1', 2, '(Var1 > 2)')
    run_evaluation(initial_source_log, 'Var2', 1.3, 1, 6)
