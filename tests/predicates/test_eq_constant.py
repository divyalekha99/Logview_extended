...
def run_evaluation(log, variable_name, variable_values, len_result_set, len_complement_result_set):
    ...
    filter = Query('query', EqToConstant(variable_name, variable_values))
...
def test_eq_predicate():
    ...
    run_evaluation(initial_source_log, 'Var1', 1, 2, 5)
    check_as_string('Var1', 1, '(Var1 in { 1 })')
    run_evaluation(initial_source_log, 'Var1', {2, 1}, 5, 2)
    check_as_string('Var1', {2, 1}, '(Var1 in { 1, 2 })')
    run_evaluation(initial_source_log, 'Var2', 1, 0, 7)
    check_as_string('Var2', 1, '(Var2 in { 1 })')
    run_evaluation(initial_source_log, 'Var2', 1.4, 1, 6)
    check_as_string('Var2', 1.4, '(Var2 in { 1.4 })')
    run_evaluation(initial_source_log, 'Var3', 'One', 6, 1)
    check_as_string('Var3', 'One', "(Var3 in { 'One' })")
